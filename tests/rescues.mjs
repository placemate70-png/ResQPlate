...
try {
  ...
  assert((await step(a,'pickup')).error,'Cannot skip pickup start')
  assert((await step(a,'start_delivery')).error,'Cannot skip pickup')
  assert((await step(a,'deliver')).error,'Cannot skip delivery start')
  assert.equal((await step(a,'start_pickup')).error,null)
  assert.equal((await step(a,'pickup')).error,null)
  assert((await step(ngo,'receive',{plates:8})).error,'Cannot receive before delivery')
  assert.equal((await step(a,'start_delivery')).error,null)
  assert((await step(b,'deliver')).error,'Unrelated volunteer denied')
  const deliveries=await Promise.all([step(a,'deliver'),step(a,'deliver')])
  assert(deliveries.every(result=>!result.error))
  assert.equal(deliveries[0].data.delivered_at,deliveries[1].data.delivered_at,'Concurrent delivery is idempotent')
